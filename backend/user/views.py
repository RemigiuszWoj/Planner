...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        if request.data.get("flag", False):
            ...
            doc1_obj = [UserSerializer(instance).data for idx in nd1 for instance in User.objects.all().filter(id=idx)]
            doc2_obj = [UserSerializer(instance).data for idx in nd2 for instance in User.objects.all().filter(id=idx)]
